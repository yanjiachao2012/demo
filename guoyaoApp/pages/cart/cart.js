loader.define(function(require, exports, module) {
    ...
    var listHeight = $(window).height() - $cartPage.find('.pay-panel').height() - $cartPage.find('.common-header').height() - $cartPage.find('.select-panel').height() - $('footer').height();


    /* var data = userobj.cstid == null ? null : {
         'cstid': userobj.cstid,
    ...
    var uilist = bui.list({
        id: "#scrollCart",
        //        url: ipAddress+"/favorites-api/favorites/appTrolley/list",
        //url: "http://localhost:8899/appTrolley/list",
        url: "json/cart.json",
        field: {
            //            data: "data.data"
            data: "pageInfo"
    ...
    function template(data) {
        ...
        $.each(data, function(index, el) {
            ...
            html += '           </div>';
            html += '           <div class="h-join">去凑单<i class="icon-next"></i></div>';
            html += '        </div>';
            ...
            //html += '            <div class="nail" style="background: url(' + picProxyUrl + el.img + ') no-repeat;background-size: contain;"></div>';
            ...
            html += '                   <div class="if-price" data-price="' + el.price + '">';
            //html += '                   <div class="if-price" data-price="' + el.info[0].price + '">';
            //html += '                       <span class="unit">¥</span><span class="int">' + parseInt(el.info[0].price) + '</span><span class="decimal">.</span><span class="float">' + ((parseFloat(el.info[0].price)).toFixed(2) + "").split('.')[1] + '</span>';
            html += '                   </div>';
            ...
            //html += '             <div class="sc-type">促销</div><div class="bui-select sc-content bui-text-hide" id="select' + el.goodsid + '" value="' + (el.info.length > 0 ? el.info[0].name : "") + '">' + (el.info.length > 0 ? el.info[0].name : "无促销活动") + '</div>';
            selectList.push({
                name: "select" + el.goodsid,
                data: el.info
            });
            numIndex++;
            html += '           </div>';
            html += '           <div class="enable-choose">' + el.sale.length + '个可选</div>';
            //html += '         <div class="enable-choose">' + el.info.length + '个可选</div>';
            html += '        </div>';
            html += '    </div>';
            html += '</li>';
    ...
    function bind() {
        ...
        $.each(numList, function(i, el) {
            numWidget.push(bui.number({
                ...
                onInput: function() {
                    alert('1')
                    var $this = $("#" + el.id).parent().parent().parent().parent();
                    $($this).find("input[type=checkbox]").prop("checked", true);
                    getCount();
                }
            }));
        ...
        $(".bui-panel-head .h-join").on("click", function() {
            router.load({
                url: "pages/userCenter/join/join.html",
                param: {
    ...
    pageview.cartInit = function() {
        listHeight = $(window).height() - $cartPage.find('.pay-panel').height() - $cartPage.find('.common-header').height() - $cartPage.find('.select-panel').height() - $('footer').height();
        uilist.refresh();
    }
